...
#%% load pretrained Domain transfer net and freeze
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i,  (real_A, real_B,_) in enumerate(train_loader):
        ...
        # Permute dimensions of real_A and real_B tensor from (N, D, H, W) to (H, N, D, W)
        # D refers to Depth, H is Height, W is Width, N is Number of batch
        real_A = real_A.permute(2, 0, 1, 3)
        ...
        # GAN loss
        # Transpose dimensions of real_A from (H, N, D, W) to (N, D, H, W) for fixing misalignment
        real_A_xz = torch.transpose(real_A, 0, 2)   
        same_A = netG_A_stacking(real_A_xz)
        
        # Transpose dimensions of same_A from (N, D, H, W) to (H, N, D, W)
        same_A_xy = torch.transpose(same_A, 0, 2);
        loss_identity_A = criterion_identity(same_A_xy, real_A)
        
        # Transpose dimensions of fake_A_xy from (H, N, D, W) to (N, D, H, W)
        ...
        # Transpose dimensions of fake_A_xyz from (N, D, H, W) to (H, N, D, W)
    ...
    PATH = 'H:\\DomainTransfer\\Case4\\checkpoints\\ck_CSFNet_TT\\%d' % epoch +'.pt'
